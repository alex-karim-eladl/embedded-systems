...
#include "cs43l22.h"
const unsigned int G5 = 0x51;
const unsigned int A5 = 0x61;
const unsigned int B5 = 0x71;
const unsigned int C5 = 0x11;
const unsigned int D5 = 0x21;
const unsigned int E5 = 0x31;
volatile unsigned int PAUSED = 0;
volatile unsigned int DCNT_2 = 0;
volatile unsigned int DCNT_3 = 0;
volatile unsigned int SWITCH = 0;
...
volatile State BUTTON_STATE = idle;
...
uint8_t t1[] = {B5, A5, G5, A5, B5, B5, B5, A5, A5, A5, B5, B5, B5, B5, A5, G5, A5, B5, B5, B5, B5, A5, A5, B5, A5, G5};
...
uint8_t t2[] = {E5, D5, C5, E5, D5, C5, C5, C5, C5, C5, D5, D5, D5, D5, E5, D5, C5};
Song HOT_CROSS_BUNS = {
    t2,
    17,
    0,
...
int SPEEDS[] = {5000000, 2500000, 1250000};

Song *CURRENT_TUNE = &MARY_HAD_A_LITTLE_LAMB;
    ...
    if (EXTI->PR & (1 << 0))
    {
        ...
        case (idle):
            if (GPIOA->IDR & (1 << 0))
            {
                ...
                BUTTON_STATE = waiting_for_timer;
            }
            break;
        case (waiting_for_timer):
            if (~GPIOA->IDR & (1U << 0))
            {
                // Now we just wait for a second click
                BUTTON_STATE = waiting_for_click_2;
                GPIOD->ODR ^= (1U << 15);
                TIM3->CNT = 1000;
                // Undo the long wait timer
                TIM2->CR1 &= ~(1U << 0);
        ...
        case (waiting_for_click_2):
            if (GPIOA->IDR & (1 << 0))
            {
                // Undo Double Click Timer
                TIM3->CR1 &= ~(1U << 0);
                GPIOD->ODR ^= (1U << 15);
                ++CURRENT_TUNE->speed_idx;
                CURRENT_TUNE->speed_idx = CURRENT_TUNE->speed_idx % 3;
                BUTTON_STATE = idle;
            }
            break;
        default:
...
// Handler for the long press check
...
int main(void)
{
    ...
    EXTI->FTSR |= 0x00001; // Enable rising edge trigger on EXTI0
    EXTI->RTSR |= 0x00001;
    ...
    {
        CURRENT_TUNE->idx = 0;
        while (CURRENT_TUNE->idx < CURRENT_TUNE->len)
        {
            i2c_write(CS43L22_REG_BEEP_FREQ_ON_TIME, CURRENT_TUNE->notes[CURRENT_TUNE->idx]);
            for (volatile int j = 0; j < SPEEDS[CURRENT_TUNE->speed_idx] || PAUSED; j++)
            {
                if (SWITCH)
                {
                    CURRENT_TUNE = CURRENT_TUNE == &MARY_HAD_A_LITTLE_LAMB ? &HOT_CROSS_BUNS : &MARY_HAD_A_LITTLE_LAMB;
                    SWITCH = 0;
                    CURRENT_TUNE->idx--;
                    break;
                }
